...
export const DetallesClienteModal = ({
  ...
  console.log(selectedDetallesCliente);
  const ubicacion = {
    latitud: 0,
    longitud: 0,
  };
  ...
  const cuotasPagadas = selectedDetallesCliente.cuotasPagadas;

  const creditoTotal = cuotaDiaria * totalCuotas;
  const valorPagado = cuotaDiaria * cuotasPagadas;

  const deudaActual = creditoTotal - valorPagado;
    ...
    <div className="w-full min-h-screen h-screen z-10 bg-black bg-opacity-50 px-2 md:px-8 flex justify-center items-center fixed top-0 left-0">
      <div className="bg-gray-200 w-[400px] p-4 rounded-md relative max-h-[90%] overflow-y-auto text-sm">
        <div className="absolute right-2 top-2 cursor-pointer bg-white rounded-[50%] p-[1px]">
          <IoIosClose size={40} onClick={() => setVerDetallesCliente(false)} />
        </div>
        ...
        <p className="uppercase font-semibold">
          CPF: <span className="font-bold">{selectedDetallesCliente.cpf}</span>
        </p>

        <p className="uppercase font-semibold">
          tlf:{" "}
          <span className="font-bold">{selectedDetallesCliente.telefono}</span>
        </p>

        <p className="uppercase font-semibold break-all text-sm">
          direccion:{" "}
          <span className="font-bold">{selectedDetallesCliente.direccion}</span>
        </p>
